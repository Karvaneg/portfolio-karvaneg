...
import type { SocialLink } from '@/types/ui-kit';
...
export function getSocialLinks(iconClassName?: IconSize): SocialLink[] {
  return Object.entries(socialData).map(([id, href]) => {
    const iconName = iconNameMap[id] || id.charAt(0).toUpperCase() + id.slice(1);
    const Icon = (icons[iconName as keyof typeof icons] ||
      icons.ExternalLink) as React.ComponentType<{ className?: string; 'aria-hidden'?: boolean }>;
